...
$connect = array(
  ...
  'domain_controllers'	=>	array('') // you domain controllers, separated on comma 192.168.1.1, 192.168.1.2
);
...
if ($ldap_user != false) {
  ...
  $row = $forum_db->query_build($query) or error(__FILE__, __LINE__);

  while ($UserID = $forum_db->fetch_row($row)) $login = $UserID[0];
  ...
  if (!empty($login)) {
    ...
    $user = $la->user()->info($form_username, array("mail","displayname","title","sAMAccountName"));
  	$user_info = array(
